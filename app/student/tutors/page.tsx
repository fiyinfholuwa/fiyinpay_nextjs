...
export default function TutorsPage() {
  ...
  useEffect(() => {
    authorizedApi<Recommendation[]>("/students/me/recommendations")
      .then(setTutors)
      .catch((error) => setToast({ type: "error", title: "Unable to load tutors", message: error instanceof Error ? error.message : "Please try again." }));
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {filteredTutors.map((tutor) => (
          <TutorCard
            ...
            bookingDisabled={subscribedTutorIds.has(tutor.user.id)}
            onBook={async () => {
              ...
              try {
                ...
                setToast({ type: "success", title: "Booking created", message: "Fund your wallet, then confirm it from My subscriptions." });
              } catch (error) {
                setToast({ type: "error", title: "Unable to book tutor", message: error instanceof Error ? error.message : "Please try again." });
              }
            }}
          />
        ))}
