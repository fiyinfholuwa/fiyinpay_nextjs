  ...
  useEffect(() => {
    ...
    const updateTimers = () => {
      ...
      assignments.forEach((assignment) => {
        ...
        const startedAt = Number(window.localStorage.getItem(storageKey) ?? Date.now());
    ...
    <WorkspacePage eyebrow="Student workspace" title="Assignments" description="Complete work from your tutors and review your grades and feedback.">
      ...
      <div className="space-y-5">{assignments.map((assignment) => {
        ...
        return <Section key={assignment.id} title={assignment.title} description={`From ${assignment.tutor.firstName} ${assignment.tutor.lastName} · ${assignment.type} · ${assignment.points} points${assignment.dueAt ? ` · Due ${new Date(assignment.dueAt).toLocaleString()}` : ""}`}>
          ...
          {assignment.type === "OBJECTIVE" && assignment.durationMinutes && !assignment.submission && <p className={`mt-4 rounded-xl px-4 py-3 text-sm font-bold ${timerPassed ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700"}`}>{timerPassed ? "Time expired" : `Time remaining: ${formatTimer(timeLeft[assignment.id] ?? assignment.durationMinutes * 60_000)}`}</p>}
          {assignment.type === "OBJECTIVE" && <div className="mt-5 space-y-2">{(assignment.options?.choices ?? []).map((choice) => <label className="flex items-center gap-3 rounded-xl border border-slate-200 p-3 text-sm text-slate-700" key={choice}><input type="radio" name={`assignment-${assignment.id}`} checked={draft.answer === choice} onChange={() => updateDraft(assignment.id, { answer: choice })} />{choice}</label>)}</div>}
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4"><div>{graded ? <p className="text-sm font-bold text-emerald-700">Grade: {assignment.submission?.score}/{assignment.points}{assignment.submission?.feedback ? ` · ${assignment.submission.feedback}` : ""}</p> : assignment.submission ? <p className="text-sm font-semibold text-blue-700">Submitted and awaiting grading</p> : expired ? <p className="text-sm font-semibold text-red-700">Submission closed</p> : <p className="text-sm text-slate-500">Not submitted</p>}</div>{!graded && !expired && <button type="button" disabled={submitting === assignment.id} onClick={(event) => submit(event, assignment)} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-blue-700 disabled:cursor-wait disabled:opacity-60">{submitting === assignment.id ? "Submitting..." : "Submit assignment"}</button>}</div>
        </Section>;
      })}</div>
    </WorkspacePage>
  );
